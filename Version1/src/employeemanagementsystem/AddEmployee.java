...
public class AddEmployee extends javax.swing.JFrame {
    ...
    private void AddEmployeeBActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_AddEmployeeBActionPerformed
        ...
        int exp = Integer.parseInt(ExperienceCB.getSelectedItem().toString());
        String city = CityCB.getSelectedItem().toString();
        String country = CountryCB.getSelectedItem().toString();
        String password = PasswordTF.getText();
        int departmentId = CountryCB.getSelectedIndex();
        ...
        String strInsert = "INSERT INTO employee_info ( name, address, email, cnic, mobile_no, employee_department_id, basic_salary, age, city, country, experience, create_date) "
                                 + "values('"+name+"','"+address+"','"+email+"','"+cnic+"','"+mobileNo+"',1,'"+salary+"','"+age+"','"+city+"','"+country+"','"+exp+"',curdate())";

        stmt.execute(strInsert);
        
        Statement stmt1 = conn.createStatement();
        String selectData = "Select * from employee_info where cnic = '"+cnic+"'";  
        ResultSet rs = stmt1.executeQuery(selectData);
        int employeeInfoId = 0;
            while(rs.next()){
       
               employeeInfoId = rs.getInt("id");
           }
        Statement stmt2 = conn.createStatement();
                
        String strInsert1 = "INSERT INTO login (email, password, user_type_id, employee_info_id)"
                                 + "values('"+email+"','Login@123',2,'"+employeeInfoId+"')";
