...
public class AddAttendance extends javax.swing.JFrame {
    ...
    public AddAttendance() {
      ...
      public AddAttendance(int employeeId) {
       initComponents();
        this.controller = new EmployeeAttendanceController();
    ...
    private void AddEmployeeBActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_AddEmployeeBActionPerformed
       try {
            ...
            // Call controller to add attendance
            boolean success = controller.addAttendance(date, timeIn, timeOut, empId);

            if (success) {
                JOptionPane.showMessageDialog(null, "Attendance Added Successfully");
                clearAll();
            } else {
                JOptionPane.showMessageDialog(null, "Failed to Add Attendance. Please try again.");
            }
        } catch (Exception ex) {
            ex.printStackTrace();
